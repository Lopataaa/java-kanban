...
class HttpPriorityTest extends http.HttpTasksTest {

    @Test
    void testGetPrioritizedTasks() throws IOException, InterruptedException {
        Task task1 = new Task("Task Early", "Early task", 0, TaskStatus.NEW,
                LocalDateTime.now().plusHours(1), 60);
        Task task2 = new Task("Task Late", "Late task", 0, TaskStatus.IN_PROGRESS,
